...
void init();
void s21_select(regex_t *compiled, int len, int argc, char **argv,
                int curr_pos);
...
regex_t *assoc_4(int *curr_pos, int argc, char *argv[], regex_t *compiled,
                 int *len);
            ...
            if (!grep.c) {
              ...
              if (grep.o && !grep.v) {
                ...
                printf("%.*s\n", (int)(pmatch[0].rm_eo - pmatch[0].rm_so),
                       &line[pmatch[0].rm_so]);
                for (int k = 0; k < len; k++) {
                  while (!regexec(compiled + k, line + appender, nmatch, pmatch,
                                  REG_NOTBOL) &&
