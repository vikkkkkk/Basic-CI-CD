...
void optionE(struct options *cats, int *BlankLine, int *edit,
             int *num_BlankLine);
...
void optionE(struct options *cats, int *BlankLine, int *edit,
             int *num_BlankLine) {
  if (cats->e || cats->E) printf("$");
  if (*BlankLine == 0) *num_BlankLine += 1;
  printf("\n");
  *BlankLine = 0;
  *edit = 1;
}
...
void PostProcessing(struct options *cats, char group, int *edit,
                    int countCats) {
  if (group != '\n') {
    *edit = 0;
...
int counting(struct options *cats) {
  int count = 0;
  count = cats->e + cats->E + cats->v + cats->n + cats->t + cats->T + cats->s +
          cats->b;
  return count;
}
